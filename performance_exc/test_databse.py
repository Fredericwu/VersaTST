...
class Write_to_database(object):
    def __init__(self,host,port,Text_Table_Name,Client_Name,Date,Disk_Type,list_data): #Key_ID,
        # self.Key_ID = Key_ID
        self.host = host
        self.port = port
    ...
    def create_db_table(self):
        ...
        #Write_to_database(self.UID,Client_Name,self.Date,Disk_Type,self.list_data)
        ...
        values = (self.Client_Name, self.Date, self.Disk_Type, self.Text_Table_Name ) #self.Key_ID, 
        cur.execute (query,values)
        
        sql_sentence = cur.execute('SELECT * FROM Index_Table')
    ...
    def SQL_text_input(self):
        ...
        try:
            ...
            table_info= '+----------------------------+' + self.Text_Table_Name + ' information' + '+------------------------+'
            print(table_info)
            
            query = f'''INSERT INTO {self.Text_Table_Name} (table_name, DRBD_type, Readwrite_type, blocksize, IOdepth, Number_of_Job, IOPS, MBPS) values (%s, %s, %s, %s, %s, %s, %s, %s)'''
            for data in self.list_data:      
                DRBD_type = data[0]
                ...
                values = (self.Text_Table_Name, DRBD_type, Readwrite_type, blocksize, IOdepth, Number_of_Job, IOPS, MBPS)
                cur.execute(query, values)
                sql_result = f'SELECT * FROM {self.Text_Table_Name}'

            cur.execute(sql_result)
            x = from_db_cursor(cur)
            utils.prt_log('', f"print table_name informastion.... \n {x}", 0)
            # print (x)
            cur.close()
            con.commit()
            con.close()
...
class Get_data_info():
    ...
    def get_iops_data_read(self):
        ...
        for l in range(len(dev_fs_value)):
            ...
            sql_result_1 = f'SELECT blocksize,IOPS FROM {self.Text_Table_Name} WHERE DRBD_type="{dev_fs_value[l]}" AND Readwrite_type="read"'
            ...
            list_value = []
        count  = len(list_all)
        table_name_end = self.Text_Table_Name + '_' + 'read' + '_' + 'IOPS'
        cur.close()
        con.commit()
        con.close()

        dict_data = {"code": 0, "msg": "", "count": count, "data": list_all ,"table_name":table_name_end}
        return dict_data
    ...
    def get_iops_data_write(self):
        ...
        for l in range(len(dev_fs_value)):
            list_key.append("dev_fs")
            list_value.append(dev_fs_value[l])
            sql_result_1 = f'SELECT blocksize,IOPS FROM {self.Text_Table_Name} WHERE DRBD_type="{dev_fs_value[l]}" AND Readwrite_type="write"'
            cur.execute(sql_result_1)
            for ii in cur:
                list_key.append(ii[0])
                list_value.append(ii[1])
        ...
        count  = len(list_all)
        table_name_end = self.Text_Table_Name + '_'  + 'write' + '_' + 'IOPS'
        cur.close()
        con.commit()
        con.close()
    ...
    def get_iops_data_randrw(self):
        ...
        for l in range(len(dev_fs_value)):
            ...
            list_value = []
        count  = len(list_all)
        table_name_end = self.Text_Table_Name + '_' + 'IOPS'
    ...
    def get_mbps_data_read(self):
        ...
        for l in range(len(dev_fs_value)):
            list_key.append("dev_fs")
            list_value.append(dev_fs_value[l])
            sql_result_1 = f'SELECT blocksize,MBPS FROM {self.Text_Table_Name} WHERE DRBD_type="{dev_fs_value[l]}" AND Readwrite_type="read"'
            cur.execute(sql_result_1)
            for ii in cur:
                list_key.append(ii[0])
                list_value.append(ii[1])
        ...
        count  = len(list_all)
        table_name_end = self.Text_Table_Name + '_'  + 'read' + '_' + 'MBPS'
        cur.close()
        con.commit()
        con.close()
    ...
    def get_mbps_data_write(self):
        ...
        for l in range(len(dev_fs_value)):
            list_key.append("dev_fs")
            list_value.append(dev_fs_value[l])
            sql_result_1 = f'SELECT blocksize,MBPS FROM {self.Text_Table_Name} WHERE DRBD_type="{dev_fs_value[l]}" AND Readwrite_type="write"'
            cur.execute(sql_result_1)
            for ii in cur:
                list_key.append(ii[0])
                list_value.append(ii[1])
        ...
        count  = len(list_all)
        table_name_end = self.Text_Table_Name + '_'  + 'write' + '_' + 'MBPS'
        cur.close()
        con.commit()
        con.close()
    ...
    def get_mbps_data_randrw(self):
        ...
        for l in range(len(dev_fs_value)):
            ...
            list_value = []
        count  = len(list_all)
        table_name_end = self.Text_Table_Name + '_' + 'MBPS'
