...
class sql_graph_histogram (object):
    def __init__(self,host,port,Table_Names,rwt,iod,nj):
        self.host = host
        self.port = port
        ...
        self.rwType_info = rwt
        self.iod = iod
        self.nj_info = nj
        self.IOPS_4K_list = ['IOPS','4k']
        self.MBPS_1M_list = ['MBPS','1M']
        # print('table_nnnnnnnnnnnn',self.table_n)
    ...
    def get_data_histogram(self,IO_MB_list):
        ...
        for i in range(len(self.rwType_info)):
            ...
            iod_info ='"{}"'.format(self.iod[0])
            bs = '"{}"'.format(self.IO_MB_list[1])
            rw = '"{}"'.format(self.rwType_info[i])
            sql_sentence = 'SELECT Text_Table_Name, DRBD_Type,' + ' ' + self.IO_MB_list[0] + ' ' + 'FROM Index_Table ,' +  self.table_n \
                + ' ' + 'WHERE Readwrite_type = ' + rw\
                + ' ' + 'AND Number_of_Job = ' + nj_info \
                + ' ' + 'AND IOdepth = ' + iod_info \
                + ' ' + 'AND blocksize =' + bs \
                + ' ' + 'AND Index_Table.Text_Table_Name =' + ' ' + self.table_n + '.table_name' \
            
            cur.execute(sql_sentence)
            file_name =  'histogram' + '-' + self.IO_MB_list[0] + '-' + self.rwType_info[i] + '-' + self.IO_MB_list[1] 
            file_name_list.append(file_name)
            for row in cur:
                self.drbd.append(row[1])
                values.append(row[2])
        # print('self.drbdddddddddddddddddddddd',self.drbd)
